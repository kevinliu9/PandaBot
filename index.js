...
bot.on('message', message => {
    ...
    if (message.content.includes('abcde')) {
        var member = bot.users.get("147668257950662656");//.send("does this work?");
    ...
    switch (args[0]) {
        ...
        case 'clear':
            ...
            break;
        case 'asdfgh':
            message.channel.send(/*"My Bot's message",*/ { files: ["https://discordapp.com/assets/1036416ac37e433260e450a69f98827c.svg"] });
            break;
        case 'babypandas':
        ...
        case 'help':
            ...
            break;
            if (args[1]) {
                message.channel.sendMessage('Please use this command properly next time!');
            }
        case 'roll':
            if (!args[1]) { message.channel.sendMessage('Please enter in a number to roll to!'); break; }
            var a = Math.floor(Math.random() * args[1]) + 1;
            if (!(a > 0 && a <= args[1])) { message.channel.sendMessage('Please enter in a number to roll to!'); break;  }
            else {  
                message.channel.sendMessage("Rolled a " + a)
                break;
            }
        case 'kick':
